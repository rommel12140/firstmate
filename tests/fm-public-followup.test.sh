...
test_rechain_delivers_second_post_on_same_thread() {
  local parent log out posts command command_log emit_path recorder_path
...
SH
  ...
  # Bash 3.2 misparses literal slashes inside quoted replacement patterns.
  # Whole path variables preserve the exact generated command under that shell.
  emit_path="$ROOT/bin/fm-public-followup-emit.sh"
  recorder_path="$parent/fakebin/record-emit"
  command=${command/"$emit_path"/"$recorder_path"}
