...
test_selected_harness_block_only() {
  ...
  assert_contains "$out" "Mode: Codex Stop-owned event notification." "codex snippet missing"
  assert_contains "$out" "bin/fm-codex-stop.sh" "codex Stop owner missing"
...
test_repair_lines() {
  ...
  assert_contains "$out" "state/.codex-notify-failure" "codex repair line omitted failure evidence"
  ...
  assert_contains "$out" "state/.codex-notify-failure" "x-mode codex repair line lost the failure evidence"
...
test_cross_harness_ordinary_continuation_and_repair_matrix() {
  ...
  out=$("$RENDER" --harness codex)
  ordinary=$(printf '%s\n' "$out" | grep -F -- '- Ordinary wake:')
  assert_contains "$ordinary" "Codex Stop adapter already owns watcher continuity" "codex ordinary-wake line lost its owner"
  assert_not_contains "$ordinary" "bin/fm-watch-checkpoint.sh" "codex ordinary wake directed a checkpoint"
  ...
  assert_contains "$out" "Codex Stop notification path" "codex recovery line lost its adapter repair"
  assert_contains "$out" "state/.codex-notify.log" "codex recovery line lost the diagnostic log"
