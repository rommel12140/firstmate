...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  assert_no_grep "make \`--intent\` preserve all relevant content" "$home/data/$id/brief.md" \
...
test_no_mistakes_dod_wording() {
  ...
  assert_grep "make \`--intent\` preserve all relevant content" "$brief" \
    "no-mistakes DOD must retain accepted Task and Scope requirements"
  assert_grep "plus any later words the captain actually said" "$brief" \
    "no-mistakes DOD must allow later captain words in --intent"
  assert_grep "never label Firstmate-written material as verbatim captain words" "$brief" \
    "no-mistakes DOD must preserve source attribution"
...
test_ask_user_escalation_format() {
  ...
  for mode in direct-PR local-only; do
    ...
    # shellcheck disable=SC2046 # land_flags emits fixture arguments.
    FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$other_id" some-proj --mode "$mode" $(land_flags "$mode") >/dev/null 2>&1
    other_brief="$home/data/$other_id/brief.md"
...
test_worker_role_scope() {
  ...
  for kind in no-mistakes direct-PR local-only scout; do
    if [ "$kind" = scout ]; then
      ...
      # shellcheck disable=SC2046 # land_flags emits fixture arguments.
      FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$kind" arbitrary-project-name --mode "$kind" $(land_flags "$kind") >/dev/null || fail "$kind scaffold failed"
    fi
