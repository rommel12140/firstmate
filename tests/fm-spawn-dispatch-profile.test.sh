...
test_worker_launch_delivers_role_scope() {
  ...
  for kind in no-mistakes direct-PR local-only scout; do
    ...
    if [ "$brief_kind" != scaffold ]; then
      ...
      elif [ "$kind" = local-only ]; then
        FM_HOME="$HOME_DIR" "$ROOT/bin/fm-brief.sh" "$id" arbitrary-project-name --mode "$kind" >/dev/null || fail "$kind scaffold failed"
      else
        FM_HOME="$HOME_DIR" "$ROOT/bin/fm-brief.sh" "$id" arbitrary-project-name --mode "$kind" --land fixture-owner/fixture-repo >/dev/null || fail "$kind scaffold failed"
      fi
      brief="$HOME_DIR/data/$id/brief.md"
      content=$(cat "$brief")
      content=${content//'{TASK}'/brief for $id}
      content=${content//'{FIRSTMATE_SPEC}'/Exercise the spawn behavior under test.}
      content=${content//'{SCOPE_MUST_CHANGE}'/The dispatch fixture.}
      content=${content//'{SCOPE_MUST_NOT_TOUCH}'/Everything outside the fixture.}
      printf '%s\n' "$content" > "$brief"
    fi
...
SH
    ...
    if [ "$brief_kind" = scaffold ] && [ "$kind" = no-mistakes ]; then
      local review_scope
      review_scope=$(awk '$0 == "## Scope review input" { emit=1; next } emit && /^## / { exit } emit { print }' "$prompt")
      assert_contains "$review_scope" "Must change: The dispatch fixture." "delivered review input lost accepted scope"
      assert_contains "$review_scope" "Must not touch: Everything outside the fixture." "delivered review input lost scope exclusions"
      assert_grep 'not verbatim captain words or independent captain authorization' "$prompt" "delivered intent contract lost attribution"
    fi
