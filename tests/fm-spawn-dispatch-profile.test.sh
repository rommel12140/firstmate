...
#!/usr/bin/env bash
set -u
# A pane path per window when the caller supplies a map, so a batch or a
# second task in one case gets its own copy instead of everyone sharing one
# worktree, which fm-spawn now refuses (bin/fm-worktree-claim-lib.sh). Batch
# members re-exec sequentially, so the last window created is the one being
# asked about.
case "$*" in
  *"#{pane_current_path}"*)
    wname=
    if [ -n "${FM_FAKE_WINDOW_NAME_FILE:-}" ] && [ -f "${FM_FAKE_WINDOW_NAME_FILE}" ]; then
      wname=$(cat "$FM_FAKE_WINDOW_NAME_FILE")
    fi
    if [ -n "${FM_FAKE_PANE_PATH_DIR:-}" ] && [ -n "$wname" ] \
       && [ -f "$FM_FAKE_PANE_PATH_DIR/$wname" ]; then
      cat "$FM_FAKE_PANE_PATH_DIR/$wname"
      exit 0
    fi
...
test_home_defaults_preserve_absolute_or_resolve_relative_paths() {
  ...
  # The second spawn is a different task, so it needs its own copy: a worktree
  # a live task already records is refused (bin/fm-worktree-claim-lib.sh).
...
test_batch_forwards_shared_profile_flags() {
  local rec id1 id2 out status wt2 pane_map
  id1=profile-batch-a-z9
  id2=profile-batch-b-z10
  rec=$(make_spawn_case profile-batch claude "$id1" "$id2")
  read_case_record "$rec"
  enable_dispatch_profile "$HOME_DIR"
  # Each batch member is a separate task and gets its own copy, the way the
  # pool would hand them out; two tasks in one worktree is refused.
  wt2="$CASE_DIR/wt-batch-b"
  git -C "$PROJ_DIR" worktree add --quiet -b wt-profile-batch-b "$wt2"
  pane_map="$CASE_DIR/pane-paths"
  mkdir -p "$pane_map"
  printf '%s\n' "$WT_DIR" > "$pane_map/fm-$id1"
  printf '%s\n' "$wt2" > "$pane_map/fm-$id2"
  export FM_FAKE_PANE_PATH_DIR="$pane_map" FM_FAKE_WINDOW_NAME_FILE="$CASE_DIR/window-name"
