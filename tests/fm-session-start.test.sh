...
test_non_pi_session_start_leaves_branch_state_untouched() {
  local rec root home fakebin out harness
  for harness in claude codex; do
  rec=$(new_world "non-pi-branch-recovery-$harness")
  IFS='|' read -r root home fakebin <<EOF
$rec
EOF
  make_fake_toolchain "$fakebin"
  ...
  out=$(run_named_harness_session_start "$harness" "$home" "$root" "$fakebin:$BASE_PATH")
  case "$out" in
  ...
  pass "$harness session start neither sweeps nor replays Pi branch state"
  done
}
