...
EOF
  ...
  for kind in ship scout; do
    ...
    grep -F 'this scaffold cannot inspect the task text' "$brief" >/dev/null \
    ...
    sed -e 's/^{TASK}$/Do the real work./' \
        -e 's/^{FIRSTMATE_SPEC}$/Verify the real work./' \
...
STUB
  ...
  for mode in no-mistakes direct-PR local-only; do
    ...
    local land_args=()
    case "$mode" in no-mistakes|direct-PR) land_args=(--land "$LAND_FIXTURE") ;; esac
    FM_HOME="$home" "$BRIEF" "$id" fixture-project --mode "$mode" ${land_args[@]+"${land_args[@]}"} >/dev/null 2>&1 \
    ...
    # Landing facts are intake-specific and covered above; promotion did not
    # previously accept a landing input. Compare the shared behavioral contract.
    sed -E '/^Landing:|^Firstmate verified this landing place|^If (the remotes|any tool)/d; s/ land=[^ ]+//; /^$/d' "$brief_dod" > "$brief_dod.normalized"
    sed '/^$/d' "$delivered_dod" > "$delivered_dod.normalized"
    cmp -s "$brief_dod.normalized" "$delivered_dod.normalized" \
...
test_spawn_and_promote_require_filled_task_subsections() {
  local rec home proj fakebin out status id brief meta intent_body spec_body scope_body authorized
...
EOF
  ...
  FM_HOME="$home" "$BRIEF" "$id" proj --mode no-mistakes --land "$LAND_FIXTURE" >/dev/null 2>&1 \
  ...
  FM_HOME="$home" "$BRIEF" "$id" proj --mode direct-PR --land "$LAND_FIXTURE" >/dev/null 2>&1 \
...
EOF
  ...
  spec_body=$(awk '$0 == "## Legacy Task review input" { emit=1; next } emit && /^## / { exit } emit { print }' "$home/data/$id/launch-brief.md")
  assert_contains "$spec_body" "Captain: Fix the legacy dispatch boundary." \
    "legacy review input lost its explicit captain attribution"
  assert_contains "$spec_body" "preserve its explicit source labels" \
    "legacy review input treated mixed authorship as a single source"
...
# Scope
Must not touch: existing task worktrees.
...
EOF
  ...
  spec_body=$(awk '$0 == "## Firstmate specification review input" { emit=1; next } emit && /^## / { exit } emit { print }' "$home/data/$id/launch-brief.md")
  assert_contains "$spec_body" "Preserve the existing compatibility path." \
    "review input lost the accepted Firstmate requirement"
  scope_body=$(awk '$0 == "## Scope review input" { emit=1; next } emit && /^## / { exit } emit { print }' "$home/data/$id/launch-brief.md")
  assert_contains "$scope_body" "Must not touch: existing task worktrees." \
    "review input lost the accepted scope constraint"
  assert_grep "not verbatim captain words or independent captain authorization" "$home/data/$id/launch-brief.md" \
    "review input mislabeled Firstmate specification"
  assert_grep "do not turn worker inventions or implementation choices into authorization" "$home/data/$id/launch-brief.md" \
    "review input admitted invented requirements"
