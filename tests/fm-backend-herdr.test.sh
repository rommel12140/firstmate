...
# bin/fm-spawn.sh run to completion against this fake; and `agent rename`
# succeeds unless FM_FAKE_HERDR_RENAME_EXIT asks it to fail. Every call is
# logged to $FM_HERDR_LOG in the same unit-separated form as
# make_herdr_fakebin.
...
# --- name_endpoint (distinct per-worker identity in herdr's own views) -------
#
# Verified live against herdr 0.7.4 on 2026-08-10 and recorded in
# docs/herdr-backend.md "Endpoint naming": `agent rename` sets BOTH `agent
# list`'s .name and `pane list`'s .label, while `pane rename` sets only .label
# and leaves .name null. Picking the wrong one is the whole failure mode this
# feature exists to avoid, so it is pinned here rather than left to a comment.

test_name_endpoint_uses_agent_rename_not_pane_rename() {
  local dir log resp fb
  ...
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p2'$'\x1f''ship:fm-alpha-a1' \
    "name_endpoint did not call 'agent rename <pane> <name>' on the right pane"
  assert_not_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''rename' \
    "name_endpoint used 'pane rename', which sets only the pane label and leaves 'agent list' .name null"
  assert_contains "$(cat "$log")" $'\x1f''--session'$'\x1f''default' \
    "name_endpoint did not route through the session-scoped CLI helper"
  pass "fm_backend_herdr_name_endpoint: names the pane with 'agent rename' (the call that sets both .name and .label)"
}
...
test_name_endpoint_preserves_rename_failure() {
  ...
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p2' \
...
# `agent rename` takes `--clear` in the name position, so a leading-dash string
# reaching herdr would risk CLEARING the name instead of setting it. Refuse
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p3'$'\x1f''ship:nameship-k1' \
  ...
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p4'$'\x1f''scout:namescout-k2' \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p3'$'\x1f''ship:concur-c1' \
    "the first concurrent worker was not named on its own pane"
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p4'$'\x1f''ship:concur-c2' \
  ...
  assert_not_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p2' \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p3' \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p3'$'\x1f''ship:batchname-b1' \
    "the batch's first worker was not named with its own task id"
  assert_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''rename'$'\x1f''w1:p4'$'\x1f''ship:batchname-b2' \
