#!/usr/bin/env bash
# Shared configured active-alert execution. Source with FM_HOME and log() set.
# The caller writes its durable failure marker before invoking wedge_alarm_notify.
# Existing away-mode defaults are preserved; FM_ALERT_TITLE labels other callers.
WEDGE_ALARM_TIMEOUT_SECS_DEFAULT=10

# These pane-independent transports are shared by the away-mode injection
# alarm and the Codex Stop adapter. Callers own when an episode is actionable;
# this library owns bounded configured channel execution.
#
...
# foot of fm-supervise-daemon.sh defaults it to "discard" in library mode.
# Other executable tests must set their recorder explicitly. The special value
...
wedge_alarm_via_osascript() {  # <summary>
  ...
  wedge_alarm_run_bounded osascript osascript -e 'on run argv' \
    ...
    -e 'end run' "$summary" "${FM_ALERT_TITLE:-firstmate: away-mode escalations WEDGED}" >/dev/null 2>&1 && return 0
...
wedge_alarm_via_herdr() {  # <summary>
  ...
  wedge_alarm_run_bounded herdr herdr notification show "${FM_ALERT_TITLE:-firstmate: away-mode escalations WEDGED}" \
