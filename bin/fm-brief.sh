...
# bin/fm-dod-lib.sh owns the shared review-checkpoint selection policy.
...
DOD=$(fm_dod_block "$MODE" "$ID" "$LAND" "$VERIFIED_DATE") || exit 1
...
$DOD
EOF
echo "scaffolded: $BRIEF (ship, mode=$MODE; replace {TASK}, {FIRSTMATE_SPEC}, {SCOPE_MUST_CHANGE}, {SCOPE_MUST_NOT_TOUCH})"
