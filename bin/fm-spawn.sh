...
#   That publication is fail-closed: a spawn that cannot write the task's
#   state/<id>.meta record exits non-zero with an error instead of reporting a
#   task no supervision record can ever find.
...
# group, so the record file is created and truncated by a simple command first,
# where `||` catches the failure. Guarding the group itself instead would make it
# a `||` operand, which suppresses set -e for every write inside it and reports
# only the trailing command's status: a spawn would then continue on a truncated
# metadata file. Either way, continuing here would report a spawned task no
# supervision record can ever find. Guarding SPAWN_META_PATH rather than the
# final record path covers the relaunch staging file too; the EXIT trap releases
# the metadata lock this exit leaves held.
: > "$SPAWN_META_PATH" || {
  echo "error: could not publish task metadata to $SPAWN_META_PATH" >&2
  exit 1
}
