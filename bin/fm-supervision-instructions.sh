...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s\n' "$prefix" 'repair the Codex Stop notification path: inspect state/.codex-notify-failure and state/.codex-notify.log, then let the next Stop retry.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: the Codex Stop adapter already owns watcher continuity; drain, handle and acknowledge the notification, then finish the turn.'
      ;;
    pi|pi-signed)
