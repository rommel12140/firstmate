...
# array expansion as an unbound variable under set -u (--no-projects seeds).
# The parallel PROJECT_ORIGINS read below stays plain: it is only reached from
# inside the loop, which an empty PROJECT_NAMES never enters.
