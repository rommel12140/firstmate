...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex uses its Stop-owned native queue adapter, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
FM_CODEX_WATCH_CHECKPOINT=180   # seconds for the explicit diagnostic checkpoint helper
