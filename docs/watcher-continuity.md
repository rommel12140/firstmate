...
Pi absorbs a delayed ordinary close whose readable queue is already empty before invoking a model; [Pi supervision branch](pi-supervision-branch.md#deterministic-replay-absorption) owns the replay evidence and limits.
...
Every settled branch prompt releases any residual grant, so an omitted or failed acknowledgement leaves the durable row available to a later drain or identity-bound Pi replay acknowledgement; a successful acknowledgement has already removed it.
