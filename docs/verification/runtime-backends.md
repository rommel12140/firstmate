...
### Codex sandboxed directory-trust refresh

Verified 2026-09-06 on macOS arm64 with `codex-cli 0.153.4`, using the same real `fm-send.sh` and inbox acknowledgement path in an isolated tmux server.
The probe launched `codex --sandbox workspace-write --ask-for-approval never` in a dedicated git lab, placed its temporary home and action file inside that lab, and accepted the native directory-trust prompt with Enter only after verifying the exact directory.
A ready Codex composer was observed before delivery.
The scoped reproduction restricted the guard to Codex with `FM_SEND_INBOX_LIVE_E2E=1 FM_SEND_INBOX_LIVE_HARNESSES=codex FM_SEND_INBOX_LIVE_TIMEOUT=120`; other harness versions above were not refreshed in this run.
The reusable refresh entry point remains `tests/fm-send-inbox-doorbell-live-e2e.test.sh`; its default launch posture differs from this sandboxed reproduction.

```text
ok - codex (codex-cli 0.153.4): the doorbell reached a real worker, which acted and acked with the mv
