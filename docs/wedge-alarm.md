...
The channel implementation is shared through `bin/fm-alert-lib.sh`; the [Codex Stop adapter](watcher-continuity.md#codex-stop-notification) uses it for notification failures.
...
The away-mode caller fires only after a genuine max-defer wedge and is rate-limited to at most once per max-defer window.
