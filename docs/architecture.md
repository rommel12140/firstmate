...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is neither provably working nor in a declared external-wait pause, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
A declared pause takes precedence over the working state inferred from an active pipeline run, but never over a run outcome: a paused crew whose run parks at a gate, finishes, or fails still surfaces immediately (`bin/fm-crew-state.sh`).
That precedence applies only under full `axi status` attribution, since the coarse runs-list fallback reports just running/completed/failed/cancelled and so cannot tell a parked gate from a still-validating run; a coarse-attributed run keeps its working verdict and its wedge escalation.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports that the crew is provably working or in a declared external-wait pause: an actively running no-mistakes step attributed to that crew's current code, an exact busy verdict from the semantic busy-state contract, or a `paused:` declaration that outranks an inferred working state.
...
In normal mode its `paused:` write and the short recheck turn-ends that follow are absorbed rather than surfacing through the no-verb path, so the pause first reaches the supervisor through that initial stale wake; away mode self-handles the routine signal and owns the later recheck.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, only when it matches the crew's branch and current code identity, then keeps that run-step authoritative even if the pane has closed, subject only to the declared-pause precedence above.
...
A pool slot frees when its lease is released, including when a task's harness session merely dies, so a later `treehouse get` can hand a live task's copy to a second task.
`fm-spawn.sh` therefore also refuses a copy that any other live task in the same home already records, and hands it straight back with a non-forced `treehouse return`; [`bin/fm-worktree-claim-lib.sh`](../bin/fm-worktree-claim-lib.sh) owns that scan and the durable record it reads.
...
A push mode carries one further intake decision, the landing place `bin/fm-brief.sh` takes as `--land <owner/repo>` and records beside the mode.
...
The spawn re-reads where the project clone's own origin pushes and refuses a recorded landing place the repository contradicts, so the intake pre-answer cannot reach a worker already wrong.
...
It refuses the same shared-copy collision before any destructive step, because returning one task's worktree would destroy the other task's work; that refusal covers scouts too, and only `--force` bypasses it.
