...
- Codex registers a `Stop` hook in `.codex/hooks.json`, anchors the executable to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, and passes the original payload to the Stop notification owner (`bin/fm-codex-stop.sh`), which invokes the shared guard when notification cannot be verified.
