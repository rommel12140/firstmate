...
It establishes that from the window inventory, matching the recorded name as text, because tmux answers a `display-message` for an absent target from the current client's own window and still exits 0, and its target selectors match by pattern rather than exactly.
