...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`, plus `--land` on a push mode), scout, secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
...
| `fm-watch-checkpoint.sh` | Run one bounded foreground watcher checkpoint for diagnostics or rollback            |
| `fm-codex-stop.sh` | Own the local Codex primary watcher lifetime and native event notification |
