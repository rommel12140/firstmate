...
Run the suite from a detached HEAD when your checkout doubles as the repo under test: several tests (teardown, secondmate harness, watcher triage) exercise the real worktree-tangle guard against this checkout, and a named non-default branch makes its banner pollute their captured output.
`git checkout --detach` at your branch tip before a suite walk, and re-attach the branch to commit.
...
Tests that need a real optional backend, an explicit opt-in, or a host toolchain their own subject requires (real herdr/zellij/cmux smoke tests, the live Pi regression, the Kimi hook's `python3` prerequisite) skip themselves and print the tool or environment gate needed to enable them, so the portable suite remains safe on machines without those tools.
