...
Resolve every ship task's concrete delivery mode, yolo posture, and verified landing place at intake, passing the mode and yolo explicitly to the brief, the spawn, and any scout promotion, which all refuse to guess, and the landing place to the brief alone, which records it for the spawn to re-verify.
...
Running, fixing, or CI states remain working, except that a worker's declared `paused:` wait can report `paused` during them; parked approval or fix-review states require the worker to follow the active gate help; passed or checks-passed is done; failed or cancelled is failed.
...
Use its scaffold as the contract, then replace every generated placeholder with a clear task description, its scope boundaries, acceptance criteria, constraints, and necessary context before dispatch or seeding.
